...
`npm.cmd run verify` now runs pure PHP authentication/JWT/URL tests, dedicated MariaDB authentication, lifecycle, wizard, and campaign-context integrations using `.env.test`, and Playwright in desktop/mobile Chromium. Browser tests inject a deterministic Google verifier only when `APP_ENV=test` and `POLITIKS_TEST_AUTH=enabled`; production configuration cannot select it accidentally.
